...
info_dict['learning_rate'] = 3e-5 #3e-4
...
def run_train(info_dict):
    #info_dict['checkpoint'] = './Report/GCN_ObjAndPredShare_NoFtExModule_LSTM-22012024-220712.pth.tar'
    ...
    subset = 'test'
    DATA_DIR = './Data'
    images_data = joblib.load(f"{DATA_DIR}/{subset}/cheapfake_{subset}_lowered_images_data.joblib")#_neural_motif
    ...
    obj_ft_dir_val = "./Data/test/PENET/VisualObjectFeatures"#Neural_Motif
    pred_ft_dir_val = './Data/test/PENET/VisualPredFeatures'#Neural_Motif
...
def run_evaluate(info_dict):
    ...
    #info_dict['checkpoint'] = './Report/GCN_ObjAndPredShare_NoFtExModule_LSTM-20122023-062715.pth.tar'
    #info_dict['checkpoint'] = './Report/GCN_ObjAndPredShare_NoFtExModule_LSTM-06012024-122619.pth.tar'
    #info_dict['checkpoint'] = './Report/GCN_ObjAndPredShare_NoFtExModule_LSTM-14012024-082201.pth.tar'
    #info_dict['checkpoint'] = './Report/GCN_ObjAndPredShare_NoFtExModule_LSTM-24012024-085338.pth.tar'
    info_dict['checkpoint'] = './Report/GCN_Cheapfake_weights_16000_cosineloss.pth.tar'
    ...
    caps_data = joblib.load(f"{DATA_DIR}/{subset}/cheapfake_{subset}_lowered_caps_data.joblib")
    df = pd.read_csv(f"{DATA_DIR}/{subset}/label_file_{subset}.csv")
    OBJ_FT_DIR = f"{DATA_DIR}/{subset}/PENET/VisualObjectFeatures" # run extract_visual_features.py to get this
    PRED_FT_DIR = f"{DATA_DIR}/{subset}/PENET/VisualPredFeatures" # run extract_visual_features.py to get this
