...
class FuzzyLayer(nn.Module):
    def __init__(self, input_dim, membership_num, device='cpu'):
    ...
    def forward(self, input_seq):
        ...
        fuzzy_membership = torch.mean(torch.exp((-1 / 2) * ((input_seq_exp - membership_miu_exp) / (membership_sigma_exp + 1e-6)) ** 2), dim=-1)
...
class FeatureExtraction(nn.Module):
    def __init__(self, info_dict, MLP_dim):
        ...
        # Initialize the Fuzzy Layer with membership_num = gcn_output_dim
        print("gcn output dim", self.gcn_output_dim)
        self.fuzzy_layer = FuzzyLayer(input_dim=self.gcn_output_dim*2, membership_num=self.gcn_output_dim*2, device=device)
                                             
        self.mlp = MLP(input_dim=self.gcn_output_dim*8, 
    ...
    def forward(self, images_objects, images_predicates, list_objects, list_predicates, edges, image_num_objects, image_num_predicates, captions_predicates_1, captions_edges_1, captions_length_1, caption_number_objects_1, caption_number_edges_1, captions_predicates_2, captions_edges_2, captions_length_2, caption_number_objects_2, caption_number_edges_2):
        ...
        # Apply fuzzy transformation on image_geb
        fuzzy_output = self.fuzzy_layer(image_geb)
        
        x = torch.cat([image_geb, fuzzy_output, caption_geb_1, caption_geb_2], dim=1)
        output = self.mlp(x)
        return output
